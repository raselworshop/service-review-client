  ...
  return (
    <section className="bg-white py-12 px-6 md:px-16 flex flex-col md:flex-row items-center">
      ...
      <div className="md:w-1/2 space-y-4">
        <h1 className="text-4xl font-bold text-gray-900">It's free to be on Service Review</h1>
      ...
      <div className="md:w-1/2 relative mt-8 md:mt-0">
        ...
        <div className="absolute top-4 left-4 bg-white p-3 rounded-lg shadow-md w-52">
          <p className="text-gray-900 font-semibold">Message from Abby:</p>
          <p className="text-gray-600 text-sm">Do you offer a flower delivery service?</p>
        </div>
        
        {/* Floating Review Box */}
        <div className="absolute bottom-4 right-4 bg-white p-3 rounded-lg shadow-md w-56">
          <div className="flex items-center space-x-1">
            {Array(5).fill().map((_, i) => (
              <span key={i} className="text-red-500">★</span>
            ))}
          </div>
          <p className="text-gray-900 font-semibold">Bloom & Co did an AMAZING job with our arrangement order!</p>
        </div>
      </div>
    </section>
  );
